...
def make_mat(mat,var):
  result=matrix(GF(2),0,3*n)
  i=0
  j=0
  while i<var:
    try:
      result=block_matrix([result,take_2_build_3(mat[0:n,:],mat[n+j+2*i,:],mat[n+j+2*i+1,:],n+2*i,n+2*i+1,2)],ncols=1)
      i += 1
...
def sweep(mat):
  i=1
  for i in range(3*n):
    print make_mat(mat,i).transpose().kernel()
